...
class Graph():
    ...
    def profile(self):
        ...
        params_flag_map = {}
        for key in self.initials:
            params_flag_map[key] = 0 #onnx参数赋值0

        self.macs = 0.0 #初始化计算量（MACs）为 0。
        self.params = 0 #初始化参数数量为 0。
        self.memory = 0 #初始化内存占用为 0
        tensors = []
        for key in self.nodemap.keys(): #遍历模型的节点 475？
            node = self.nodemap[key] #获取节点对象，尝试打印conv_size！！！！
            itensors = []
            _params = 0
            _memory = 0
            max_sparsity = 0 #初始化最大稀疏度为 0
            block_sparsity = {'blocksize': (1, 1), 'blockratio': 0, 'ratio': 0} #初始化块稀疏度信息为空字典。
            for input in node.input:
                tensor = self.tensormap[input]
                itensors.append(tensor.get_valueorshape())
                if input in self.initials:
                    if params_flag_map[input] == 0:
                        elesize = volume(self.tensormap[input].get_shape()) #获取张量体积（元素数量）。
                        _params += elesize #累加到当前节点的参数数量中
                        _memory += elesize * self.tensormap[input].get_elementsize() #累加到当前节点的内存占用中。
                    params_flag_map[input] += 1
                if tensor.sparsity is not None and tensor.sparsity['ratio'] > max_sparsity: #如果输入张量存在稀疏度信息且稀疏度较大
                    max_sparsity = tensor.sparsity['ratio'] #更新最大稀疏度。
                    block_sparsity = tensor.sparsity #更新块稀疏度信息
            otensors = []
            for output in node.output:
    ...
    def print_node_map(self, f: str = None, metric='MACs', exclude_ops=None):
        ...
        conv_params_dict={}
        for key in self.nodemap.keys():
            node = self.nodemap[key] #尝试在这打印！！！！！！
            if node.op_type =="Conv":
                # print(key)
                channel_shape_stride_pad = [node.inshape[1],node.kernel_shape[0],node.kernel_shape[0],node.strides[0],node.pads[0],tuple2str(node.inshape, splitch),tuple2str(node.outshape, splitch)]
                conv_params_dict.setdefault(key,[]).append(channel_shape_stride_pad)
                # print(node.outshape[1])
                # print(node.kernel_shape)
                # print(node.pads)
                # print(node.strides)
            if exclude_ops is not None and node.op_type in exclude_ops:
                continue
            row = [key, self.nodemap[key].op_type] #在此判断是卷积归为一类,这个位置没有batch
            ...
            ptable.append(row)
        print("-------------------------------------start print----------------------------")
        print(conv_params_dict)
        with open('conv_params.txt', 'a') as file:
            #写入标题行
            file.write("conv_name,channel,shape,shape,stride,pad,input_shape,output_shape\n")
    
            # 遍历字典，将每组参数写入文件
            for conv_name,value in conv_params_dict.items():
                channel, shape, shape,stride, pad, input_shape, output_shape = value[0]
                file.write(f"{conv_name},{channel},{shape},{shape},{stride},{pad},{input_shape},{output_shape}\n") 
        # 临时字典，用于存储值与键的映射关系
        temp_dict = {}
        # 遍历原始字典
        for key, value in conv_params_dict.items():
            value_tuple = tuple(value[0])
            if value_tuple not in temp_dict:
                temp_dict[value_tuple] = []
            temp_dict[value_tuple].append(key)
        # 打印归类后的键和相同值
        with open('conv_params.txt', 'a') as file:
            for value, keys in temp_dict.items():
                file.write(f"Value: {value[:5]}, Keys: {keys},Length:{len(keys)}\n")
                print(f"Value: {value[:5]}, Keys: {keys},Length:{len(keys)}")
